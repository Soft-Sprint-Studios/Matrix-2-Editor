                ...
                if (dispFaces.Any())
                {
                    using (var dw = new StreamWriter(dispPath))
                    {
                        foreach (var f in dispFaces)
                        {
                            ...
                            dw.WriteLine($"texture2 {(string.IsNullOrWhiteSpace(f.Displacement.Texture2Name) ? "NULL" : f.Displacement.Texture2Name)}");
                            ...
                            dw.WriteLine($"alphas {string.Join(" ", f.Displacement.Alphas.Select(a => a.ToString(System.Globalization.CultureInfo.InvariantCulture)))}");
                            dw.WriteLine("}");
                        }
                    }
                }
