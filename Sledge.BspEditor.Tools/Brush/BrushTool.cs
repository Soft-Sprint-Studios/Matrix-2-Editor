...
namespace Sledge.BspEditor.Tools.Brush
{
    ...
    public class BrushTool : BaseDraggableTool, ISettingsContainer
    {
        ...
        private Draw3DMode _mode3d = Draw3DMode.None;
        private bool _draggedThisStroke;
        private int _heightReferenceScreenY;
        private float _heightReferenceUnits;
        ...
        public override async Task ToolDeselected()
        {
            ...
            Reset3DMode(box.State.Action != BoxAction.Idle);
        ...
        {
            if (e.KeyCode == Keys.Enter && box.State.Action == BoxAction.Drawn)
            {
                Confirm(document, viewport);
                e.Handled = true;
                return;
            }
        ...
        #region 3D brush creation (Source 2 style)

        private void Reset3DMode(bool clearBox)
        {
            _mode3d = Draw3DMode.None;
            _draggedThisStroke = false;
            if (clearBox) box.State.Action = BoxAction.Idle;
        }

        /// <summary>
        /// Intersect a ray with the horizontal plane at the given Z. Returns null if the ray never crosses it.
        /// </summary>
        ...
        /// <summary>Fallback point when the ray misses the plane.</summary>
        private static Vector3 FallbackPoint(Line ray)
        {
            return ray.Start + (ray.End - ray.Start) * 256;
        }

        /// <summary>
        /// World units per screen pixel at a given distance from a perspective camera.
        /// </summary>
        private static float UnitsPerPixelAtDistance(PerspectiveCamera camera, MapViewport viewport, float distance)
        {
            var fovRad = (float)(Common.MathHelper.DegreesToRadians(camera.FOV));
            return 2f * distance * (float)Math.Tan(fovRad / 2f) / Math.Max(1, viewport.Height);
        }

        /// <summary>Round a height to the grid step unless snapping is bypassed with Alt.</summary>
        private float SnapHeight(MapDocument document, float height)
        {
            if (Sledge.Shell.Input.KeyboardState.Alt) return height;
            var gridData = document.Map.Data.GetOne<GridData>();
            var snapEnabled = gridData?.SnapToGrid != false;
            if (!snapEnabled) return height;

            var stepVec = gridData?.Grid?.AddStep(Vector3.Zero, Vector3.One) ?? Vector3.One;
            var step = Math.Max(1f, Math.Abs(stepVec.Z));
            return (float)Math.Round(height / step) * step;
        }

        private void PublishBoxDimensions(BoxState state)
        {
            var b = new Box(state.Start, state.End);
            var label = b.IsEmpty() ? "" : $"{b.Width:0} x {b.Length:0} x {b.Height:0}";
        ...
        {
            // Second click in height mode: commit the brush
            if (_mode3d == Draw3DMode.Height && e.Button == MouseButtons.Left)
            {
                Confirm(document, viewport);
                e.Handled = true;
                return;
            ...
            {
                point = IntersectHorizontalPlane(ray, 0f) ?? FallbackPoint(ray);
            }

            point = SnapIfNeeded(point);

            box.RememberedDimensions = null; // fresh draw, don't reuse old thickness
            box.State.Action = BoxAction.Drawing;
            box.State.Start = point;
            box.State.End = point;

            _mode3d = Draw3DMode.Base;
            _draggedThisStroke = false;
        ...
        {
            // Click without dragging: abort the stroke cleanly
            if (_mode3d == Draw3DMode.Base && !_draggedThisStroke)
            {
                Reset3DMode(true);
                PublishBoxDimensions(box.State);
        ...
        {
            if (_mode3d != Draw3DMode.Base || e.Button != MouseButtons.Left) return;
            _draggedThisStroke = true;
            e.Handled = true;
        }
        ...
        protected override void DragMove(MapDocument document, MapViewport viewport, PerspectiveCamera camera, ViewportEvent e)
        {
            ...
            // Base stays flat on the plane the stroke started on
            var planeZ = box.State.Start.Z;
            var p = IntersectHorizontalPlane(ray, planeZ);
            if (p.HasValue) p = SnapIfNeeded(p.Value);
            else p = FallbackPoint(ray);

            box.State.End = new Vector3(p.Value.X, p.Value.Y, planeZ);
            PublishBoxDimensions(box.State);
            e.Handled = true;
        }
        ...
        protected override void DragEnd(MapDocument document, MapViewport viewport, PerspectiveCamera camera, ViewportEvent e)
        {
            ...
            // Normalise start/end and guarantee non-zero thickness like the 2D flow does
            box.State.FixBounds();
            FixZeroThickness(document);

            box.State.Action = BoxAction.Drawn;

            _mode3d = Draw3DMode.Height;
            _heightReferenceScreenY = e.Y;
            _heightReferenceUnits = box.State.End.Z - box.State.Start.Z;
        ...
        {
            if (_mode3d != Draw3DMode.Height) return;

            var centre = (box.State.Start + box.State.End) / 2;
            var distance = Math.Max(64f, (camera.Position - centre).Length());
            var unitsPerPixel = UnitsPerPixelAtDistance(camera, viewport, distance);

            // Screen-up is world-up: moving mouse up extrudes taller.
            var deltaPixels = _heightReferenceScreenY - e.Y;
            var height = SnapHeight(document, _heightReferenceUnits + deltaPixels * unitsPerPixel);

            box.State.End = new Vector3(box.State.End.X, box.State.End.Y, box.State.Start.Z + height);
            PublishBoxDimensions(box.State);
            e.Handled = true;
        }

        private void FixZeroThickness(MapDocument document)
        {
            var dimensions = box.State.End - box.State.Start;
            var gridData = document.Map.Data.GetOne<GridData>();
            var sg = gridData?.Grid as SquareGrid;
            var step = sg?.Step ?? 16f;
            if (step < 1f) step = 16f;

            box.State.End = new Vector3(
                NewThickness(dimensions.X, box.State.End.X),
                NewThickness(dimensions.Y, box.State.End.Y),
                NewThickness(dimensions.Z, box.State.End.Z));

            float NewThickness(float thickness, float endValue) => thickness == 0 ? endValue + step : endValue;
        }
        ...
        private IMapObject GetBrush(MapDocument document, Box bounds, UniqueNumberGenerator idg)
        {
            ...
            var ti = document.Map.Data.GetOne<ActiveTexture>()?.Name ?? "trigger";
        ...
        {
            if (box.State.Action != BoxAction.Drawn)
            {
                Reset3DMode(false);
                return;
            }
            ...
            Reset3DMode(false);
            PublishBoxDimensions(box.State); // clear the W x L x H readout
