...
namespace Sledge.BspEditor.Tools.Vertex.Tools
{
    ...
    public class VertexPointTool : VertexSubtool
    {
        ...
        // ------------------------------------------------------------------
        // 3D vertex dragging state
        // ------------------------------------------------------------------
        private const int PerspPickWidth = 6;
        private VertexPoint _perspMouseDownAnchor;
        private bool _dragging3D;
        private Sledge.DataStructures.Geometric.Plane _dragPlane;
        private Vector3 _dragAnchorOriginal;
        private Vector3 _current3DDelta;
                ...
                foreach (var solid in _vertices.Keys)
                {
                    ...
                    foreach (var face in obj.Faces.ToList())
                    {
                        ...
                        for (int i = 0; i < face.Vertices.Count - 2; i++)
                        {
                            ...
                            tris.Add(new MutableFace(faceVerts, face.Texture));
                        }

                        var newFaces = new List<MutableFace> { tris.First() };
        ...
        #region 3D point picking and dragging

        /// <summary>
        /// Screen-space pick of vertex points in the 3D view. Unlike the inherited
        /// BaseDraggableTool pipeline, this does not require the pick ray to hit map geometry first.
        /// </summary>
        private List<VertexPoint> PickPoints(PerspectiveCamera camera, MapViewport viewport, ViewportEvent e)
        {
            var l = camera.EyeLocation;
            var pos = new Vector3((float)l.X, (float)l.Y, (float)l.Z);
            var p = new Vector3(e.X, e.Y, 0);
            var d = PerspPickWidth;

            return (from point in GetVisiblePoints()
        ...
        {
            _perspMouseDownAnchor = null;

            var toggle = KeyboardState.Ctrl;
            var clicked = PickPoints(camera, viewport, e);

            Select(clicked, toggle);
            if (clicked.Any())
            {
                _perspMouseDownAnchor = clicked[0];
                e.Handled = true;
            }
        }
        ...
        protected override void MouseMove(MapDocument document, MapViewport viewport, PerspectiveCamera camera, ViewportEvent e)
        {
            ...
            if (document.Selection.IsEmpty && !_vertices.Any()) return;

            var picked = PickPoints(camera, viewport, e);

            foreach (var pt in GetVisiblePoints()) pt.IsHighlighted = false;
            if (picked.Any()) picked[0].IsHighlighted = true;

            viewport.Control.Cursor = picked.Any() ? Cursors.SizeAll : Cursors.Default;
        }
        ...
        protected override void DragStart(MapDocument document, MapViewport viewport, PerspectiveCamera camera, ViewportEvent e)
        {
            ...
            if (_perspMouseDownAnchor == null || !_perspMouseDownAnchor.IsSelected) return;

            StartDrag3D(camera, _perspMouseDownAnchor);
            viewport.AquireInputLock(this);
            e.Handled = true;
        }

        private void StartDrag3D(PerspectiveCamera camera, VertexPoint anchor)
        {
            _dragging3D = true;
            _dragAnchorOriginal = anchor.Position;
            _current3DDelta = Vector3.Zero;
            _dragPlane = new Sledge.DataStructures.Geometric.Plane(camera.Direction.Normalise(), anchor.Position);

            foreach (var p in GetVisiblePoints().Where(x => x.IsSelected)) p.IsDragging = true;
        }

        protected override void DragMove(MapDocument document, MapViewport viewport, PerspectiveCamera camera, ViewportEvent e)
        {
            if (!_dragging3D || e.Button != MouseButtons.Left) return;

            var (rs, re) = camera.CastRayFromScreen(new Vector3(e.X, e.Y, 0));
            var ray = new Line(rs, re);
            var isect = _dragPlane.GetIntersectionPoint(ray, true);
            if (!isect.HasValue) return;

            _current3DDelta = ComputeSnappedDelta(isect.Value);
            ApplyDrag3D(_current3DDelta);
            e.Handled = true;
        }

        private Vector3 ComputeSnappedDelta(Vector3 target)
        {
            var raw = target - _dragAnchorOriginal;
            if (KeyboardState.Alt) return raw;

            var doc = GetDocument();
            var gridData = doc?.Map.Data.GetOne<GridData>();
            if (gridData?.Grid == null || gridData?.SnapToGrid == false) return raw;

            // Snap the anchor's target position to the grid lattice so vertices
            // land on-grid when they started on-grid, keeping faces rigid.
            var snapped = gridData.Grid.Snap(_dragAnchorOriginal + raw);
            return snapped - _dragAnchorOriginal;
        }

        private void ApplyDrag3D(Vector3 delta)
        {
            var selected = GetVisiblePoints().Where(x => x.IsSelected).Distinct().SelectMany(x => x.GetStandardPointList()).ToList();
        ...
        private void EndDrag3D()
        {
            ...
            foreach (var point in pts) point.Move(_current3DDelta);
        ...
        #region Point clicking and dragging (2D)
